...
export default function CreateEventUpdateForm(props) {
  const {
    id: idProp,
    createEvent: createEventModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = createEventRecord
      ? { ...initialValues, ...createEventRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setDetailedInfo(cleanValues.detailedInfo);
    ...
    setOrgDesc(cleanValues.orgDesc);
    setOrgAddress(cleanValues.orgAddress);
    setErrors({});
  };
  const [createEventRecord, setCreateEventRecord] =
    React.useState(createEventModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CreateEvent, idProp)
        : createEventModelProp;
      setCreateEventRecord(record);
    };
    queryData();
  }, [idProp, createEventModelProp]);
  React.useEffect(resetStateValues, [createEventRecord]);
  const validations = {
    name: [],
    description: [],
    detailedInfo: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CreateEvent.copyOf(createEventRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CreateEventUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || createEventModelProp)}
